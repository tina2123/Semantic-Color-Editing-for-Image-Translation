...
def run(image, mode, sample_c=1.3,  num_samples=3, sample_step=100, color='Blue',category=88):
    ...
    ckpt_folder = os.path.join(current_dir, 'ckpt')

    args.val_data_dir = image
    args.sample_c = sample_c
    args.num_samples = num_samples
   
    pretrained_diffusion.chosen_color = color
    pretrained_diffusion.category= int(category)

    pretrained_diffusion.file_path= os.path.basename(image.name)
    args.chosen_color = color
    ...
    if mode == 'sketch':
        ...
        args.model_path = os.path.join(ckpt_folder, 'base_edge.pt')
        args.sr_model_path = os.path.join(ckpt_folder, 'upsample_edge.pt')      

    elif mode == 'mask':
        args.mode = 'coco'
        args_up.mode = 'coco'
        args.model_path = os.path.join(ckpt_folder, 'base_mask.pt')
        args.sr_model_path = os.path.join(ckpt_folder, 'upsample_mask.pt')
    ...
    if args.mode == 'coco':
        ...
        data_dict = {"ref":label_tensor.unsqueeze(0).repeat(args.num_samples, 1, 1, 1)}
    data_dict['chosen_color'] = chosen_color
...
demo = gr.Interface(fn=run, inputs=[
    gr.inputs.File(label="Input Sketch", type="file"),  
    ...
    gr.inputs.Radio(label="Choose Color", choices=['bluish', 'greenish', 'reddish'],default="bluish"),
    gr.inputs.Number(label="Category ID", default=1),
